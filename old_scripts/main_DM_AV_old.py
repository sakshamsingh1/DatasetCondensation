...
from additional_utils.train_utils_DM import eval_synthetic_comb, realBatchCombPT, adjust_lr_DM
...
def main(args):
    interval = 10
    eval_it_pool = np.arange(interval, args.Iteration+1, interval).tolist()
    ...
    def get_aud_images(c, n): # get random n images from class c
        ...
        if args.input_modality == 'a' or args.input_modality == 'av':
            idx_aud = dst_train[idx_shuffle][0].to(args.device)
        if args.input_modality == 'v' or args.input_modality == 'av':
            idx_img = dst_train[idx_shuffle][1].to(args.device)
    ...
    net_frame = builder.build_frame(
        ...
        weights=args.weights_frame, pretrained=True)
    ...
    for it in range(args.Iteration+1):
        ...
        if it%10 == 0:
            ...
            print(f'{get_time()} iter = {it:05d}, loss = {loss_avg:.4f}, lr = {optimizer_comb.param_groups[0]["lr"]:.6f}, patience = {patience}')
        
        if (it+1) % args.lr_train_drop_freq==0:
        ...
        if min_loss >= loss_avg:
            ...
            min_loss = loss_avg
            args.it = it
            aud_syn_eval, image_syn_eval, label_syn_eval = copy.deepcopy(aud_syn.detach()), copy.deepcopy(image_syn.detach()), copy.deepcopy(label_syn.detach()) # avoid any unaware modification
            final_acc = eval_synthetic_comb(args, aud_syn_eval, image_syn_eval, label_syn_eval, valloader, testloader)
...
if __name__ == '__main__':
    ...
    args.id = f'{args.method}_{args.dataset}_mod-{args.input_modality}_ipc-{args.ipc}_init-{args.init}_loss:{args.DM_loss_type}'
